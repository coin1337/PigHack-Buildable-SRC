...
public class ConfigManager
    ...
    public String config = "pighack-r/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                PigHack.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    PigHack.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("pighack-r").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("pighack-r/" + name + "/")) ? "pighack-r/" + name + "/" : "pighack-r/config/";
        PigHack.friendManager.onLoad();
    ...
    public void saveConfig(String name) {
        ...
        this.config = "pighack-r/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        PigHack.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("pighack-r/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("pighack-r", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(PigHack.moduleManager.modules);
        this.features.add(PigHack.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        PigHack.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            PigHack.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
