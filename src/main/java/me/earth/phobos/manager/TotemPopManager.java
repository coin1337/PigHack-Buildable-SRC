...
public class TotemPopManager
    ...
    public void onUpdate() {
        if (this.notifications.totemAnnounce.passedMs(this.notifications.delay.getValue().intValue()) && this.notifications.isOn() && this.notifications.totemPops.getValue().booleanValue()) {
            for (EntityPlayer player : this.toAnnounce) {
                ...
                Command.sendOverwriteMessage(""+ ChatFormatting.RED + player.getName() + " popped " + ChatFormatting.WHITE + this.getTotemPops(player) + ChatFormatting.RED + " Totem" + (this.getTotemPops(player) == 1 ? "" : "s") + ".", playerNumber, this.notifications.totemNoti.getValue());
    ...
    public void init() {
        this.notifications = PigHack.moduleManager.getModuleByClass(Notifications.class);
    }
    ...
    public void onDeath(EntityPlayer player) {
        if (this.getTotemPops(player) != 0 && !player.equals(TotemPopManager.mc.player) && this.notifications.isOn() && this.notifications.totemPops.getValue().booleanValue()) {
            ...
            Command.sendOverwriteMessage( "" + ChatFormatting.RED + player.getName() + " died after popping " + ChatFormatting.WHITE + this.getTotemPops(player) + ChatFormatting.RED + " Totem" + (this.getTotemPops(player) == 1 ? "" : "s") + ".", playerNumber, this.notifications.totemNoti.getValue());
            this.toAnnounce.remove(player);
        }
