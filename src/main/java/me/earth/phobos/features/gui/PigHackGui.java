...
public class PigHackGui
        extends GuiScreen {
    private static PigHackGui phobosGui;
    private static PigHackGui INSTANCE;

    static {
        INSTANCE = new PigHackGui();
    }
    ...
    public static PigHackGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new PigHackGui();
        }
        return INSTANCE;
    ...
    public static PigHackGui getClickGui() {
        return PigHackGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : PigHack.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    PigHack.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
