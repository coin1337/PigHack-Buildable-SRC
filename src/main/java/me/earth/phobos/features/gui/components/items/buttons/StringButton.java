...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            PigHack.textManager.drawStringWithShadow(this.currentString.getString() + PigHack.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) PigHackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            PigHack.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " " + "\u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) PigHackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
