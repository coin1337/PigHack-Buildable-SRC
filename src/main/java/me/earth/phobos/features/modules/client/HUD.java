...
import me.earth.phobos.features.modules.render.ToolTips;
...
public class HUD
    ...
    private final Setting<WaterMark> watermark = this.register(new Setting<WaterMark>("Logo", WaterMark.PIGHACK, "WaterMark"));
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "ZipWare.CC"));
    private final Setting<Boolean> modeVer = this.register(new Setting<Object>("Version", Boolean.valueOf(false), v -> this.watermark.getValue() != WaterMark.PIGHACK));
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "PigHack", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public void onUpdate() {
        for (Module module : PigHack.moduleManager.sortedModules) {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PIGHACK: {
                this.renderer.drawString("PigHack" + (this.modeVer.getValue() != false ? " " + PigHack.MODVER : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case ZIPWARE: {
                this.renderer.drawString("ZipWare.CC" + (this.modeVer.getValue() != false ? " " + PigHack.MODVER : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() != false ? " "  + PigHack.MODVER: ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
            }
        }
        ...
        if (this.arrayList.getValue().booleanValue()) {
            ...
            if (this.renderingUp.getValue().booleanValue()) {
                for (int i = 0; i < (this.alphabeticalSorting.getValue() != false ? PigHack.moduleManager.alphabeticallySortedModules.size() : PigHack.moduleManager.sortedModules.size()); ++i) {
                    module = this.alphabeticalSorting.getValue() != false ? PigHack.moduleManager.alphabeticallySortedModules.get(i) : PigHack.moduleManager.sortedModules.get(i);
                    text3 = module.getDisplayName() + "\u00a77" + (module.getDisplayInfo() != null ? " [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]" : "");
                    moduleColor = PigHack.moduleManager.moduleColorMap.get(module);
                ...
                for (int i = 0; i < (this.alphabeticalSorting.getValue() != false ? PigHack.moduleManager.alphabeticallySortedModules.size() : PigHack.moduleManager.sortedModules.size()); ++i) {
                    module = this.alphabeticalSorting.getValue() != false ? PigHack.moduleManager.alphabeticallySortedModules.get(PigHack.moduleManager.alphabeticallySortedModules.size() - 1 - i) : PigHack.moduleManager.sortedModules.get(i);
                    text3 = module.getDisplayName() + "\u00a77" + (module.getDisplayInfo() != null ? " [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]" : "");
                    moduleColor = PigHack.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            ...
            if (this.serverBrand.getValue().booleanValue()) {
                text2 = grayString + "Server brand " + "\u00a7f" + PigHack.serverManager.getServerBrand();
            ...
            if (this.potions.getValue().booleanValue()) {
                for (PotionEffect effect : PigHack.potionManager.getOwnPotions()) {
                    text = this.altPotionsColors.getValue() != false ? PigHack.potionManager.getPotionString(effect) : PigHack.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue().booleanValue()) {
                text2 = grayString + "Speed " + "\u00a7f" + PigHack.speedManager.getSpeedKpH() + " km/h";
                this.renderer.drawString((String) text2, width - (this.renderer.getStringWidth((String) text2) + 2), height - 2 - (i += 10), this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(height - i) : this.color, true);
            }
            ...
            if (this.tps.getValue().booleanValue()) {
                String text4 = grayString + "TPS " + "\u00a7f" + PigHack.serverManager.getTPS();
            ...
            text3 = grayString + "Ping " + "\u00a7f" + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : (long) PigHack.serverManager.getPing()) + (this.MS.getValue() != false ? "ms" : "");
            ...
            if (this.tps.getValue().booleanValue()) {
                String text5 = grayString + "TPS " + "\u00a7f" + PigHack.serverManager.getTPS();
            ...
            fpsText = grayString + "FPS " + "\u00a7f" + Minecraft.debugFPS;
            text3 = grayString + "Ping " + "\u00a7f" + PigHack.serverManager.getPing();
            if (this.renderer.getStringWidth(text3) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue().booleanValue()) {
                    this.renderer.drawString(text3, width - (this.renderer.getStringWidth(text3) + 2), 2 + i++ * 10, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2 + i * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            PigHack.notificationManager.handleNotifications(height - (i + 16));
        } else {
            PigHack.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        String text6 = (this.direction.getValue() != false ? PigHack.rotationManager.getDirection4D(false) + " " : "") + (this.coords.getValue() != false ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "pig own u" + HUD.mc.player.getDisplayNameString() + " :3";
                break;
            }
            case CUSTOM: {
            ...
            default: {
                text = text + "hello " + HUD.mc.player.getDisplayNameString();
            }
        }
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (PigHack.serverManager.isServerNotResponding()) {
            String text = (this.lag.getValue() == LagNotify.GRAY ? "\u00a77" : "\u00a7c") + "Server not responding: " + MathUtil.round((float) PigHack.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
