...
import me.earth.phobos.features.modules.pvptools.AutoCrystal;
...
public class PistonAura
    ...
    private final Setting<breakTypes> breakType = this.register(new Setting<breakTypes>("Logo", breakTypes.SWING, "WaterMark"));

    private final Setting<Double> enemyRange = this.register(new Setting<Double>("Range", 4.9, 0.0, 6.0));
    private final Setting<Double> blocksPerTick = this.register(new Setting<Double>("BlocksPerTick", 4.0, 0.0, 20.0));
    private final Setting<Double> startDelay = this.register(new Setting<Double>("Start Delay", 4.0, 0.0, 20.0));
    private final Setting<Double> stuckDetector = this.register(new Setting<Double>("StuckCheck", 35.0, 0.0, 200.0));
    private final Setting<Double> pistonDelay = this.register(new Setting<Double>("PistonDelay", 2.0, 0.0, 20.0));
    private final Setting<Double> crystalDelay = this.register(new Setting<Double>("CrystalDelay", 4.0, 0.0, 20.0));
    private final Setting<Double> hitDelay = this.register(new Setting<Double>("Hit Delay", 4.0, 0.0, 20.0));
    private final Setting<Double> supBlocksDelay = this.register(new Setting<Double>("supBlocksDelay", 4.0, 0.0, 20.0));
    private final Setting<Double> maxYincr = this.register(new Setting<Double>("CrystalDelay", 3.0, 0.0, 5.0));

    private final Setting<Boolean> rotate = this.register(new Setting<Boolean>("Rotate ", true));
    private final Setting<Boolean> blockPlayer = this.register(new Setting<Boolean>("TrapPlayer", true));
    private final Setting<Boolean> confirmBreak = this.register(new Setting<Boolean>("NoGlitchBreak", true));
    private final Setting<Boolean> confirmPlace = this.register(new Setting<Boolean>("NoGlitchPlace ", false));
    private final Setting<Boolean> allowFastMode = this.register(new Setting<Boolean>("allowFastMode ", true));
    private final Setting<Boolean> betterPlacement = this.register(new Setting<Boolean>("betterPlacement", false));
    private final Setting<Boolean> antiWeakness = this.register(new Setting<Boolean>("antiWeakness", false));
    private final Setting<Boolean> chatMsg = this.register(new Setting<Boolean>("ChatMSG", true));


    public PistonAura() {
        super("PistonAura", "PistonAura", Module.Category.COMBAT, true, false, false);
    }
    ...
    private boolean placeBlock(BlockPos pos, int step, double offsetX, double offsetZ) {
        ...
        if (PigHack.moduleManager.isModuleEnabled("AutoCrystal")) {
    ...
    private EntityPlayer findClosestTarget() {
        ...
        for (EntityPlayer entityPlayer : playerList) {
            ...
            if (PigHack.friendManager.isFriend(entityPlayer.getName())) {
                continue;
            }
    ...
    private void printChat(String text, Boolean error) {
        Command.sendMessage((error ? ColorMain.getDisabledColor() : ColorMain.getEnabledColor()) + text);
    }
