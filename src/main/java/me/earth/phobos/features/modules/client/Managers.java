...
public class Managers
    ...
    public Setting<String> command = this.register(new Setting<String>("Command", "PigHack+"));
    public Setting<TextUtil.Color> bracketColor = this.register(new Setting<TextUtil.Color>("BColor", TextUtil.Color.LIGHT_PURPLE));
    ...
    @Override
    public void onLoad() {
        PigHack.commandManager.setClientMessage(this.getCommandMessage());
    }
        ...
        if (event.getStage() == 2) {
            ...
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    PigHack.holeManager.settingChanged();
                }
                PigHack.commandManager.setClientMessage(this.getCommandMessage());
            }
        }
