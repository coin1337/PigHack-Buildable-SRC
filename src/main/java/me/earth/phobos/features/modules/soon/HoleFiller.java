...
public class HoleFiller
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            PigHack.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            PigHack.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = PigHack.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(PigHack.holeManager.getMidSafety());
            }
        }
        object = PigHack.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(PigHack.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && PigHack.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
