...
public class ClickGui
    ...
    public Setting<Integer> rainbowHue = register(new Setting("RainbowHue", 0, 0, 255, v -> rainbow.getValue()));
    public Setting<Integer> rainbowBrightness = register(new Setting("RaiBrightness", 0, 0, 255, v -> rainbow.getValue()));
    public Setting<Integer> rainbowSaturation = register(new Setting("RaiSaturation", 0, 0, 255, v -> rainbow.getValue()));
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                PigHack.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to \u00a7a" + PigHack.commandManager.getPrefix());
            }
    ...
    public void onEnable() {
        mc.displayGuiScreen(new PigHackGui());
    }

    @Override
    public void onLoad() {
        if (this.colorSync.getValue().booleanValue()) {
            PigHack.colorManager.setColor(Colors.INSTANCE.getCurrentColor().getRed(), Colors.INSTANCE.getCurrentColor().getGreen(), Colors.INSTANCE.getCurrentColor().getBlue(), this.hoverAlpha.getValue());
        } else {
            PigHack.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        }
        PigHack.commandManager.setPrefix(this.prefix.getValue());
    }

    @Override
    public void onTick() {
        if (!(ClickGui.mc.currentScreen instanceof PigHackGui)) {
            this.disable();
        }
    }
