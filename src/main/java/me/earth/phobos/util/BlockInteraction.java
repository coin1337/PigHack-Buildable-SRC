...
public class BlockInteraction {
    ...
    public static void placeBlockScaffold(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!BlockInteraction.canBeClicked(neighbor) || eyesPos.squareDistanceTo(hitVec = new Vec3d((Vec3i)neighbor).add(0.5, 0.5, 0.5).add(new Vec3d(side2.getDirectionVec()).scale(0.5))) > 18.0625) continue;
            BlockInteraction.faceVectorPacketInstant(hitVec);
            BlockInteraction.processRightClickBlock(neighbor, side2, hitVec);
            Wrapper.getPlayer().swingArm(EnumHand.MAIN_HAND);
            BlockInteraction.mc.rightClickDelayTimer = 4;
            return;
        }
    }

    private static float[] getLegitRotations(Vec3d vec) {
        Vec3d eyesPos = BlockInteraction.getEyesPos();
    ...
    public static void faceVectorPacketInstant(Vec3d vec) {
        float[] rotations = BlockInteraction.getLegitRotations(vec);
    ...
    private static void processRightClickBlock(BlockPos pos, EnumFacing side, Vec3d hitVec) {
        BlockInteraction.getPlayerController().processRightClickBlock(Wrapper.getPlayer(), BlockInteraction.mc.world, pos, side, hitVec, EnumHand.MAIN_HAND);
    }

    public static boolean canBeClicked(BlockPos pos) {
        return BlockInteraction.getBlock(pos).canCollideCheck(BlockInteraction.getState(pos), false);
    }

    private static Block getBlock(BlockPos pos) {
        return BlockInteraction.getState(pos).getBlock();
    }
    ...
    public static boolean checkForNeighbours(BlockPos blockPos) {
        if (!BlockInteraction.hasNeighbour(blockPos)) {
            for (EnumFacing side : EnumFacing.values()) {
                BlockPos neighbour = blockPos.offset(side);
                if (!BlockInteraction.hasNeighbour(neighbour)) continue;
                return true;
            }
            return false;
        }
